...
import { useGetSurchargesQuery, useGetZoneQuery } from "@/store/slice/apiSlice";
import { Filter } from "lucide-react";
import { useState } from "react";

import { ZoneColumns } from "../table/columns/zone-column";
import { SurchargeColumns } from "../table/columns/surcharge-column";

export default function SurchargeTableView() {
  ...
  const { data, isLoading } = useGetSurchargesQuery({});
  ...
  return (
    <div>
      ...
      {isLoading && <div>...Loading all surcharge </div>}
      {/* Table */}
      {data?.data?.surcharges && (
        <AppTable columns={SurchargeColumns} data={data?.data?.surcharges} />
      )}
    </div>
  );
}
