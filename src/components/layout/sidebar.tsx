...
import CreateQuoteModal from "@/components/modals/RequestQuoteModal";
import {
  LayoutDashboard,
  Package,
  Map,
...
// Sentinel href for menu items that open a modal instead of navigating.
// Using a literal href (rather than adding a whole onClick-item variant to
// MenuItem) keeps every existing item's shape unchanged; the render loop
// below special-cases this one value.
const GET_QUOTE_ACTION = "__get_quote_modal__";
...
export default function Sidebar({
    ...
    userRole === "ADMIN"
      ...
      : userRole === "ENTERPRISE"
        ? {
            ...
            otherMenuList: enterpriseOtherMenuList,
          }
        : { menuList: customerMenuList, otherMenuList: customerOtherMenuList };

  // Active-state matching is "most specific href wins", not "first prefix
  // match wins" — otherwise a menu item whose href is itself a path-prefix
  // of a sibling's href (e.g. "My Tickets" → /dashboard/tickets and
  // "My Claims" → /dashboard/tickets/claims) both light up any time you're
  // on the more specific one, since /dashboard/tickets/claims genuinely
  // does start with /dashboard/tickets/. Scanning every href up front and
  // picking the single longest match makes that structurally impossible,
  // regardless of how many items happen to nest under each other.
  const bestMatchHref = useMemo(() => {
    const allHrefs: string[] = [];
    for (const item of [...menuList, ...otherMenuList]) {
      if (item.href !== GET_QUOTE_ACTION) allHrefs.push(item.href);
      for (const child of item.children ?? []) {
        if (child.href !== GET_QUOTE_ACTION) allHrefs.push(child.href);
      }
    }
    let best = "";
    ...
    return best;
  }, [pathname, menuList, otherMenuList]);
  ...
  const canSee = (item: MenuItem): boolean => {
    ...
    if (userRole === "ADMIN" && item.subRoles)
      return item.subRoles.includes(subRole);
    if (userRole === "ENTERPRISE" && item.enterpriseRoles)
      return item.enterpriseRoles.includes(enterpriseRole);
  ...
  const renderItems = (items: MenuItem[]) =>
    filterItems(items).map((item) => {
      const isModalAction = item.href === GET_QUOTE_ACTION;
      // Only the single most specific href match across the whole menu is
      // ever active — see bestMatchHref above for why a plain prefix check
      // here isn't enough once any item's href nests under a sibling's.
      const isActive = !isModalAction && item.href === bestMatchHref;
          ...
          {hasKids && isOpen && (
            <div className="ml-4 mt-1 space-y-0.5 pl-3 border-l-2 border-gray-100">
              {item.children!.map((child) => {
                const childActive = child.href === bestMatchHref;
                return (
                  <button
  ...
  return (
    <>
      ...
      />
      <CreateQuoteModal isOpen={quoteModalOpen} setIsOpen={setQuoteModalOpen} />
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="flex flex-wrap gap-1 mt-2">
                ...
                {user?.role === "ADMIN" && user?.adminSubRole && (
                  <span
                    className={`inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-medium capitalize ${
                      SUB_ROLE_BADGE[user.adminSubRole] ??
                ...
                {user?.role === "ENTERPRISE" && user?.enterpriseRole && (
                  <span
                    className={`inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-medium capitalize ${
                      SUB_ROLE_BADGE[user.enterpriseRole] ??
                      "bg-gray-50 text-gray-600 border border-gray-200"
                    }`}
                  >
                    {user.enterpriseRole
