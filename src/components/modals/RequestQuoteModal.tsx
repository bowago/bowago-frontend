...
type QuoteResponse = {
  ...
  appliedDiscount: {
    type?: string;
    label?: string;
    // Backend field is discountAmount, not amount — this mismatch meant
    // the Discount row on the Review step always showed "—" even when a
    // contract rate or promo genuinely reduced the price.
      ...
      {!useCustomDimension && selectedBox && (
        <div className="bg-blue-50 border border-blue-100 rounded-xl px-4 py-3 text-xs text-blue-700">
          ...
          <p className="text-blue-500">
            Max <strong>{maxWeight} kg</strong> per box &nbsp;·&nbsp; Best for:{" "}
            {selectedBox.bestFor}
          </p>
          {values.cartons > 1 && (
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="w-full">
        ...
        {rows([
          ...
          {
            label: "Discount",
            value: quote?.appliedDiscount?.discountAmount
              ? `-${formatMoney(quote.appliedDiscount.discountAmount)}${
    ...
    <Dialog.Root open={isOpen} onOpenChange={handleClose}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-50 bg-white rounded-2xl shadow-xl w-full max-w-xl p-6 max-h-[90vh] flex flex-col">
          ...
          <div className="shrink-0">{stepperEl}</div>
          <form
