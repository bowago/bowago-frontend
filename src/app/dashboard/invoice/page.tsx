          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 mb-6">
            <StatCard
              ...
              value={
                isLoading
                  ? "..."
                  : formatMoney(summary?.totalRevenueNaira, currency)
              }
              label="Total Revenue"
              trend={0}
              withTrend={false}
            ...
            />
            <StatCard
              icon={<LucideReceiptText className="w-6 h-6 text-orange-500" />}
              iconBg="bg-orange-50"
              value={
                isLoading
                  ? "..."
                  : formatMoney(summary?.pendingRevenueNaira, currency)
              }
              label={`${summary?.pendingInvoices ?? 0} Pending Invoices`}
              trend={0}
              withTrend={false}
              delay={80}
            />
            <StatCard
              icon={<LucideReceiptText className="w-6 h-6 text-red-400" />}
              iconBg="bg-red-50"
              value={
                isLoading
                  ? "..."
                  : formatMoney(summary?.refundedNaira, currency)
              }
              label={`${summary?.refundedCount ?? 0} Refunds`}
              trend={0}
              withTrend={false}
              delay={160}
            />
            <StatCard
              icon={<LucideReceiptText className="w-6 h-6 text-blue-500" />}
              iconBg="bg-blue-50"
